...
$(document).on('click', '.flight-price-choose button', function() {
	...
	if (userInput['children'] > 0) {
		var children_baseprice = parseInt($(this).parents('.flight-item').find(
				'.detail-fare.children .price')
			.text().replaceAll('.', ''));
	...
	$(this).parents('.flight-item').find('.flight-detail-item:first-child .flight-detail-wrap').each(
		function() {
			...
			detail_flight['number'] = $(this).find('.detail-flight span:first-child p:nth-child(2)')
				.text();
			detail_flight['date'] = $(this).find('.detail-from span:last-child p:nth-child(2)')
		.text();
	...
	if (checkInfoStatus == true) {
		...
		if (confirm_booking == true) {
			...
			bill['adults_baseprice'] = flightTicketChooseInfo["adults_baseprice"][0] + (
				flightTicketChooseInfo[
					"adults_baseprice"][1] == undefined ? 0 : flightTicketChooseInfo[
					"adults_baseprice"][1]);
			...
			if (userInput['children'] > 0) {
				...
				bill['children_price'] = flightTicketChooseInfo["children_price"][0] + (
					flightTicketChooseInfo[
						"children_price"][1] == undefined ? 0 : flightTicketChooseInfo[
						"children_price"][1]);
				bill['children_names'] = [];
			...
			if (userInput['infants'] > 0) {
				...
				bill['infants_baseprice'] = flightTicketChooseInfo["infants_baseprice"][0] + (
					flightTicketChooseInfo[
						...
						"infants_baseprice"][1]);
				bill['infants_price'] = flightTicketChooseInfo["infants_price"][0] + (
					flightTicketChooseInfo[
						"infants_price"][1] == undefined ? 0 : flightTicketChooseInfo[
						"infants_price"][1]);
				bill['infants_names'] = [];
				for (customerIndex; customerIndex < parseInt(userInput['adults']) + (parseInt(userInput[
