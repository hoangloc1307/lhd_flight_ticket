...
function ShowData(sortFunction) {
    ...
    for (let i in tickets) {
        ...
        if (userInput['children'] > 0) {
            for (let j in tickets[i].travelerPricings) {
                if (tickets[i].travelerPricings[j].travelerType == "CHILD") {
                    var children_price_new = parseInt(tickets[i].travelerPricings[j].price.base) + parseInt(tickets[i]
        ...
        if (userInput['infants'] > 0) {
            for (let j in tickets[i].travelerPricings) {
                if (tickets[i].travelerPricings[j].travelerType == "HELD_INFANT") {
                    var infants_price_new = parseInt(tickets[i].travelerPricings[j].price.base) + parseInt(tickets[i]
                        .travelerPricings[j].price.base) * 0.1;
                    var infants_fee = parseInt(tickets[i].travelerPricings[j].price.total - tickets[i].travelerPricings[
                        j].price.base);
        ...
        tickets_text += "<li class='total'>" + NumberWithCommas((adults_price_new + adults_fee) * userInput['adults']) +
            "</li>";
        tickets_text += "</ul>";
        if (userInput['children'] > 0) {
    ...
    if (userInput.type == 'roundtrip') {
        ...
        for (let i in tickets2) {
            ...
            if (userInput['children'] > 0) {
                ...
                for (let j in tickets2[i].travelerPricings) {
                    if (tickets2[i].travelerPricings[j].travelerType == "CHILD") {
                        ...
                        tickets_text2 += "<li class='total'>" + NumberWithCommas((children_price_new + children_fee) *
                            userInput['children']) + "</li>";
                        break;
                    }
                }
            ...
            if (userInput['infants'] > 0) {
                ...
                for (let j in tickets2[i].travelerPricings) {
                    if (tickets2[i].travelerPricings[j].travelerType == "HELD_INFANT") {
                        ...
                        tickets_text2 += "<li class='total'>" + NumberWithCommas((infants_price_new + infants_fee) *
                            userInput['infants']) + "</li>";
                        break;
                    }
                }
            ...
            tickets_text2 += "<span class='total-price' baseprice='" + tickets[i].price.total + "'><span>" +
                NumberWithCommas(total_price_ticket) +
                "</span>VND </span>";
...
$(document).on('click', '.flight-price-choose button', function() {
    ...
    var adults_baseprice = NumberCommasToInt($(this).parents('.flight-item').find('.detail-fare.adults .price')
        .text());
    ...
    if (userInput['infants'] > 0) {
        ...
        var infants_fee = NumberCommasToInt($(this).parents('.flight-item').find('.detail-fare.infants .taxes')
            .text());
    ...
    $(this).parents('.flight-item').find('.flight-detail-item:first-child .flight-detail-wrap').each(
        function() {
            ...
            detail_flight['number'] = $(this).find('.detail-flight span:first-child p:nth-child(2)')
                .text();
            detail_flight['departure_date'] = $(this).find('.detail-from span:last-child p:nth-child(2)')
                .text();
    ...
    if (number_ticket_chose == number_ticket) {
        ...
        for (var i in flightTicketChooseInfo['adults_baseprice']) {
            adults_price += flightTicketChooseInfo['adults_baseprice'][i] + flightTicketChooseInfo['adults_fee']
                [i];
        }
        ...
        if (userInput['children'] > 0) {
            ...
            for (var i in flightTicketChooseInfo['children_baseprice']) {
                children_price += flightTicketChooseInfo['children_baseprice'][i] + flightTicketChooseInfo[
                    'children_fee'][i];
            }
        ...
        if (userInput['infants'] > 0) {
            ...
            price_text += "</div>";
            price_text += "<p class='total'>" + NumberWithCommas(infants_price * userInput['infants']) + "</p>";
            price_text += "</div>";
        }
