							...
							<ul class="box-sort">
								...
								<li class="timeup" onclick="ShowData(SortDepartureTime)">
									<label for="timeup" class="label-radio">
										<input type="radio" id="timeup" class="sort" name="sort"
											value="_timeup" />
										<span class="label-circle"> </span>
...
$(document).on('click', '.button-step button:nth-child(2)', function() {
	...
	if (checkInfoStatus == true) {
		...
		if (confirm_booking == true) {
			...
			//Đổ dữ liệu
			var goDetailHTML = "<div class='title-flight'> " + GetCityNameByIATA(userInput['origin']) +
				"<i class='fas fa-long-arrow-alt-right'></i>" + GetCityNameByIATA(userInput[
					'destination']) + "</div>";
			...
			if (userInput['type'] == 'roundtrip') {
				var backDetailHTML = "<div class='title-flight'> " + GetCityNameByIATA(userInput[
						'destination']) +
					"<i class='fas fa-long-arrow-alt-right'></i>" + GetCityNameByIATA(userInput[
						'origin']) + "</div>";
			...
			$('.finish-payment .contact-detail .phone p:nth-child(2)').text($('input[name="phone"]')
				.val());
			$('.finish-payment .contact-detail .email p:nth-child(2)').text($('input[name="email"]')
				.val());
			$('.finish-payment .contact-detail .address p:nth-child(2)').text($('input[name="address"]')
				.val());
			$('.finish-payment .contact-detail .request p:nth-child(2)').text($('textarea[name="note"]')
				.val());
			...
			for (customerIndex; customerIndex < parseInt(userInput['adults']) + parseInt(userInput[
				...
				priceHTML += "<p>" + (customerIndex + 1) + "</p>";
				priceHTML += "<p>" + customerNames[customerIndex] + "</p>";
				priceHTML += "<p>" + NumberWithCommas(flightTicketChooseInfo['children_price'][0] + (
					isNaN(
						flightTicketChooseInfo['children_price'][1]) ? 0 :
					flightTicketChooseInfo[
						'children_price'][1])) + "</p>";
